...
Perancangan alur kerja terintegrasi berfokus pada transformasi prosedur manual menjadi alur digital yang mengalir secara otomatis dari titik transaksi bisnis ke repositori arsip. Prinsip dasarnya mengacu pada konsep \textit{event-triggered archival} dan \textit{metadata auto-capture}, di mana sistem tidak lagi menjadi tempat \textit{input ulang}, melainkan penerima dan pengolah data yang sudah tervalidasi.

Tiga prinsip kunci dalam desain workflow terintegrasi:
\begin{itemize}[leftmargin=*]
    \item \textbf{Single Entry, Multiple Use:} Data hanya dimasukkan sekali pada sistem sumber (seperti ERP, Pengadaan, atau Manajemen Aset). Sistem kearsipan mengambil data tersebut secara terprogram.
    \item \textbf{Rule-Based Routing:} Persetujuan dan validasi diarahkan secara otomatis berdasarkan hierarki, nilai transaksi, atau klasifikasi risiko, bukan melalui email atau chat manual.
    \item \textbf{Explicit Exception Handling:} Setiap alur harus memiliki jalur cadangan (\textit{fallback}) yang jelas jika terjadi kegagalan sistem, \textit{timeout}, atau penolakan validasi.
\end{itemize}

\begin{warnbox}
\textbf{Catatan Batasan Materi:} Desain di level ini \textbf{tidak membahas} spesifikasi teknis API, konfigurasi database, atau pemilihan middleware. Fokus tetap pada \textbf{logika bisnis, trigger event, dan alur persetujuan} yang dapat dipetakan dalam notasi standar.
\end{warnbox}
...
\textcircled{\textbackslash} & Gateway (Exclusive) & Keputusan bercabang (\textit{Jika Disetujui / Jika Direvisi}) \\
...
Berdasarkan praktik terbaik dan ekosistem sistem yang telah berjalan di PLN, terdapat tiga pola integrasi alur kerja yang paling relevan untuk optimalisasi kearsipan:

\begin{enumerate}[leftmargin=*]
    \item \textbf{Trigger-Based Auto-Capture:} Status tertentu dalam sistem operasional secara otomatis memicu pembuatan draft arsip + metadata inti.
    \item \textbf{Parallel Digital Routing:} Dokumen yang memerlukan validasi lintas unit dikirim secara simultan ke semua pihak terkait, dengan sistem mengunci versi final setelah persetujuan mayoritas/unanimous tercapai.
    \item \textbf{SLA-Driven Escalation:} Jika validasi melampaui batas waktu yang ditetapkan, sistem secara otomatis mengeskalasi ke atasan berikutnya atau mencatatnya sebagai \textit{exception} dalam log kepatuhan.
\end{enumerate}
...
% Total cycle time
\node[below=2.1cm of g1, font=\small\bfseries, text=green!50!black] {Target Cycle Time: $<$ 1 Hari Kerja};
% Legend
...
\node[right=0.08cm of lt, font=\tiny] {Trigger};
\end{tikzpicture}%
}
...
\begin{itemize}[nosep]
    \item \textbf{Trigger:} Input laporan di sistem E-HSE $\rightarrow$ status \texttt{Submitted}
...
\midrule
\textbf{Kelengkapan} & Apakah setiap trigger memiliki task \& end event yang jelas? & $\square$ Ya / $\square$ Tidak \\
...
\begin{outputbox}
\textbf{Output Wajib Bab 3:} \textit{Diagram BPMN 2.0 (To-Be)} + \textit{Peta Trigger Integrasi}. Dokumen ini akan dinilai berdasarkan logika alur, kejelasan otomasi, dan keselarasan dengan kriteria 4.3.2.
\end{outputbox}
...
\begin{itemize}[leftmargin=*]
    \item \textbf{System Triggers (Pemicu Otomatis):} Di posisi ini manusia tidak selamanya mengawali inisiatif. SOP menjabarkan peristiwa, jadwal, atau pergantian status apa di dalam aplikasi operasional sistem yang seketika memicu (\textit{trigger}) sistem manajemen persuratan/kearsipan untuk bangkit dan menyedot (\textit{auto-capture}) laporan transaksi secara otomatis.
...
\begin{tikzpicture}[
  ...
  gateway/.style={diamond, draw=gray!70, fill=yellow!8,
    minimum width=1.3cm, minimum height=1.3cm, align=center, font=\scriptsize\bfseries, inner sep=2pt, aspect=2},
...
% Total cycle
\node[font=\normalsize\bfseries, text=red!70!black] at (10.5, -7.2) {Total Cycle Time Tipikal: 3--5 Hari Kerja};
...
\node[startstyle] at (2.0, 1.8) (s2) {Trigger};
...
% Total cycle
\node[font=\normalsize\bfseries, text=green!50!black] at (10.5, -7.2) {Total Cycle Time: $<$ 1 Hari Kerja (Mayoritas Otomatis)};
...
\node[right=0.15cm of lg3, font=\scriptsize] {Trigger / Event};
...
\section{Komponen Wajib SOP Terintegrasi (Berdasarkan PARBICA Guideline 2)}
Untuk memastikan SOP terintegrasi ini dapat dieksekusi secara teknis sekaligus 100\% patuh pada tata kelola kearsipan (\textit{Good Governance}), dokumen wajib dirumuskan berdasarkan metodologi identifikasi tata kelola informasi PARBICA yang dijabarkan ke dalam 8 komponen inti berikut:
...
\midrule
1 & Tujuan \& Ruang Lingkup & Mengatur alur arsip arsip perintah kerja (SPK) pemeliharaan GI dari trigger hingga penyimpanan final \\
...
4 & System Trigger \& Metadata & Status \textit{Selesai} di ERP $\rightarrow$ auto-capture 12 field metadata (termasuk Lokasi Box Fisik jika hibrida) \\
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Aktivasi Pemicu (*System Trigger*):} Berbeda dengan tradisi konvensional di mana staf logistik sibuk memfotokopi rimbunan kertas untuk diserahkan ke unit arsip, pada SOP ini pemicunya murni teknis, yaitu: \textbf{ketika status tender berubah menjadi "Finalize Award"} pada dasbor vendor di portal sistem E-Proc PLN.
    \item \textbf{Orkestrasi Mesin (*Automated Action*):} Trigger di atas serta-merta membangkitkan agen mesin untuk bertindak gesit (\textit{auto-capture}) mengekstrak metadata dokumen, mengunci spesifikasi, dan mematenkan nominal rupiah menjadi satu bundel digital \textit{hash-locked} (direkatkan oleh kalkulasi kriptografi agar nominal tidak bisa diakali manusia belakangan).
    \item \textbf{\textit{Human-in-the-Loop} (Intervensi Manusia):} Manajer Logistik PLN tidak disuruh menolak-tarik kardus dokumen. Dia cukup membuka dasbor aplikasi gawai E-Office, membaca ringkasan *draft* BAST tersebut (dengan SLA batas tunggakan \textit{review} hanya maksimal 16 jam kerja), dan mengetuk "Otorisasi" untuk memicu pemberian *Tanda Tangan Elektronik* (TTE). Manusia berfokus murni pada kewenangan dan akuntabilitas keputusannya, bukan mengelola map administrasinya.
    \item \textbf{Klasifikasi \& Eksekusi Retensi:} Murni sedetik pasca persetujuan TTE diketuk, repositori otomatis melempar arsip tersebut ke map rahasia, menandainya dengan takdir jadwal pemusnahan \textbf{10 tahun ke depan} (\textit{retention period}), sekaligus mencekal visibilitas akses baca (*RBAC Restricted*) pagi panitia lelang di luar proyek ini.
    \item \textbf{Jalur Eskalasi Darurat (*Exception Path*):} Jika tiba-tiba API sistem peladen TTE Nasional mengalami \textit{downtime crash}, alur pengesahan arsip akan ditunda rapi di \textit{Waiting Terminal} sembari sistem menembakkan \textit{alert} proaktif ke ponsel Admin IT pusat. Tidak ada naskah yang "boleh diloloskan manual tanpa direkam", memastikan legalitas absolut PP PSTE tanpa adanya toleransi *bypass*.
\end{enumerate}

Melalui rajutan ketujuh orkestrasi di atas, peserta lokakarya diharapkan langsung menyadari bahwa kekuatan sakti dari *"SOP IT Kearsipan Terintegrasi"* berpusat pada penyerahan seluruh beban mengangkut berkas birokrasi ke pundak tangguh mesin logik, sementara sumber daya manusia (SDM) PLN akhirnya dialokasikan ke porsi analisis, pembuatan keputuan strategis dan eksekusi misi di lapangan.
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Haram Input Manual:} Field krusial (\texttt{PLN\_DOC\_ID}, \texttt{TIMESTAMP\_CREATE}, \texttt{INTEGRITY\_HASH}) harus di-\textit{generate} otomatis oleh mesin, dilarang diketik manual guna membatasi \textit{human-error}.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Gunakan template struktur 7 komponen yang telah disediakan.
