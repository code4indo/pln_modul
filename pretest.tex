...
\centering
Pre-test ini \textbf{bukan} penilaian kelulusan. Tujuannya adalah \textbf{mengukur titik awal pengetahuan} peserta agar fasilitator dapat menyesuaikan kedalaman materi dan pendampingan selama sesi pelatihan. Jawablah sejujurnya berdasarkan pemahaman Anda saat ini.
\end{plnbox}
...
\begin{enumerate}[label=\Alph*., nosep]
    \item Titik di mana proses mengalami kemacetan/penumpukan sehingga memperlambat keseluruhan alur
...
4 & Saya mengetahui apa yang dimaksud dengan ``\textit{trigger} otomatis'' dalam konteks integrasi antar-sistem (misalnya: satu status di Sistem A memicu aksi di Sistem B). & $\square$ & $\square$ & $\square$ & $\square$ \\[0.3cm]
5 & Saya dapat menjelaskan mengapa metadata arsip penting untuk \textit{retrieval} (pencarian ulang) dan kepatuhan audit. & $\square$ & $\square$ & $\square$ & $\square$ \\
\bottomrule
\end{tabularx}
\end{center}
\renewcommand{\arraystretch}{1.0}
...
3 & \textbf{A} & Terminologi proses bisnis dasar -- titik kemacetan alur kerja \\
