...
{\large PT PLN (Persero) -- Pusat Pendidikan dan Pelatihan}\\[3cm]
\begin{figure}[h]
\centering
\fbox{\parbox{3.5cm}{\centering\vspace{1cm}{\large\color{plnblue}\textbf{LOGO PLN}}\vspace{1cm}}}
\end{figure}
\vfill
{\small Dokumen ini dirancang sebagai panduan mandiri dan workshop. Kompilasi dengan pdflatex/xelatex.}
\end{titlepage}
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Tabel} menyajikan matriks, rubrik penilaian, dan data terstruktur -- gunakan sebagai referensi cepat.
    \item \textbf{Diagram TikZ/gambar} mengilustrasikan konsep kunci -- setiap diagram disertai caption multi-baris dengan penjelasan dan sumber.
    \item \textbf{Catatan kaki} (footnote) berisi sitasi regulasi dan standar menggunakan format Chicago 17th edition.
\end{itemize}
\end{plnbox}

\begin{plnbox}[Cara Menggunakan Handout Ini]
\begin{enumerate}[leftmargin=*]
    \item \textbf{Sebelum pelatihan:} Baca Pendahuluan (bab tanpa nomor) untuk memahami konteks, target kompetensi, dan arsitektur keamanan terintegrasi. Gunakan Glosarium di akhir dokumen sebagai referensi istilah teknis.
    \item \textbf{Saat pelatihan:} Ikuti pembahasan Bab 1--4 secara berurutan (kumulatif). Kerjakan Aktivitas Workshop di setiap bab (30 menit/bab). Hasil workshop menjadi bagian dari Portofolio yang dinilai.
    \item \textbf{Studi kasus integratif:} Bab 5 menguji pemahaman holistik -- baca skenario kebocoran arsip kontrak dan analisis dari 4 perspektif pilar keamanan.
...
\vspace{0.5cm}
\noindent\small\textit{Catatan: Seluruh materi dalam handout ini dirancang agar dapat dipahami secara mandiri oleh Manajemen Atas tanpa perlu membaca Modul 4.1--4.3. Istilah teknis yang belum familiar dapat ditemukan di Glosarium di bagian akhir dokumen.}
...
Dua konsep kunci yang menjadi landasan modul ini perlu dipahami sejak awal. Pertama, \textbf{Aset Informasi Kritis} adalah informasi yang kehilangan, kerusakan, atau pengungkapannya dapat menimbulkan dampak serius terhadap kelangsungan operasional, kepatuhan regulasi, atau reputasi organisasi. Dalam konteks PLN, aset informasi kritis mencakup kontrak pembangkit, laporan keuangan teraudit, dokumen RUPS, dan data pelanggan. Kedua, \textbf{Klasifikasi Arsip} adalah pengelompokan arsip berdasarkan tingkat sensitivitas dan dampak keamanannya. Modul ini menggunakan empat tingkat klasifikasi yang selaras dengan regulasi nasional dan praktik korporasi PLN: \textbf{Vital} (kerusakan berdampak nasional), \textbf{Rahasia} (kerusakan berdampak regional/unit), \textbf{Terbatas} (kerusakan berdampak operasional terbatas), dan \textbf{Publik} (tidak ada dampak signifikan).
...
\begin{itemize}[leftmargin=*]
    ...
    \item Prasyarat: Lulus Modul 4.3, memahami klasifikasi arsip dasar, familiar dengan sistem SAP (sistem perencanaan sumber daya perusahaan) dan Single Sign-On (SSO) organisasi
...
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{1.5cm} X X @{}}
\toprule
\textbf{Modul} & \textbf{Judul \& Cakupan Utama} & \textbf{Relevansi dengan Modul 4.4} \\
\midrule
\rowcolor{rowalt}
4.1 & \textbf{Merancang Potensi Integrasi Sistem, Prosedur, dan Teknologi Kearsipan Eksisting.} Peserta mampu merancang peta kondisi eksisting, matriks potensi integrasi, dan skenario integrasi terpadu. & Menyediakan pemetaan aset dan sistem yang menjadi objek perlindungan keamanan dalam Modul 4.4. \\
\rowcolor{white}
4.2 & \textbf{Perancangan Arsitektur Sistem Terintegrasi.} Peserta mampu merancang diagram arsitektur, titik integrasi antar subsistem (penciptaan, penggunaan, pemeliharaan, penyusutan), dan spesifikasi teknis/fungsional. & Menyediakan arsitektur sistem yang menjadi infrastruktur bagi kontrol keamanan (RBAC, audit trail) dalam Modul 4.4. \\
\rowcolor{rowalt}
4.3 & \textbf{Optimalisasi Prosedur dan Alur Kerja Terintegrasi.} Peserta mampu mengevaluasi prosedur eksisting, merancang alur kerja baru, dan menganalisis potensi peningkatan efektivitas. & Menyediakan prosedur dan alur kerja operasional yang harus dilindungi oleh kebijakan keamanan dalam Modul 4.4. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[htbp]
\centering
\makebox[\textwidth][c]{\includegraphics[scale=1.08]{gambar/kerangka_pikir.png}}
\caption{Kerangka Berpikir Modul 4.4 -- Pemetaan Struktur Materi terhadap Target Kompetensi dan Kriteria Penilaian\\
\small Diagram ini menunjukkan bagaimana keempat bab (Bab 1--4) dirancang secara kumulatif dan terintegrasi untuk memenuhi kriteria penilaian 4.4.1 s.d.\ 4.4.4, berlandaskan kerangka regulasi nasional dan standar internasional, serta di-capstone oleh Studi Kasus (Bab 5) yang menguji pemahaman holistik peserta.}
...
\small Sumber: Wikimedia Commons -- Role-based access control
(\url{https://commons.wikimedia.org/wiki/File:Role-based_access_control.svg})}
...
Hak akses tidak diberikan kepada individu, melainkan kepada peran (role) yang terikat pada:
\begin{itemize}[leftmargin=*]
    \item Jabatan/Fungsi: Otomatis \textit{provisioning} saat mutasi/promosi, \textit{de-provisioning} saat pensiun/resign.
    \item Single Sign-On (SSO): Autentikasi terpusat mengurangi risiko berbagi kata sandi dan celah akses manual.
...
Skema akses ditentukan oleh perpotongan peran organisasi dan tingkat sensitivitas arsip. Tabel \ref{tab:matriks-rbac} menggunakan role/jabatan aktual di lingkungan PLN yang dikelompokkan berdasarkan fungsi organisasi:
...
\multicolumn{5}{@{}l}{\cellcolor{plnblue!15}\textbf{Operasional -- Unit Kearsipan \& Tata Usaha}} \\
\cmidrule(l){1-5}
\rowcolor{rowalt}
...
\multicolumn{5}{@{}l}{\cellcolor{plnblue!15}\textbf{Fungsional -- HR, Hukum, IT}} \\
\cmidrule(l){1-5}
\rowcolor{rowalt}
...
\multicolumn{5}{@{}l}{\cellcolor{plnblue!15}\textbf{Manajemen -- Pusat \& Unit Induk}} \\
\cmidrule(l){1-5}
\rowcolor{white}
GM / SRM Unit Induk & R & R, U & R, U, A & R, U, A \\
\rowcolor{rowalt}
EVP/ SVP/ VP Pusat & R & R, U & R, U, A & R, U, A \\
\multicolumn{5}{@{}l}{\cellcolor{plnblue!15}\textbf{Pengawasan -- SPI, GCG, Cyber Security}} \\
\cmidrule(l){1-5}
\rowcolor{white}
...
\multicolumn{5}{@{}l}{\cellcolor{plnblue!15}\textbf{Sistem -- Otomasi \& Integrasi}} \\
\cmidrule(l){1-5}
\rowcolor{rowalt}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Validasi Segregation of Duties. Manajemen Atas memastikan tidak ada satu peran yang memiliki kombinasi hak akses yang berlebihan -- misalnya, satu orang tidak boleh sekaligus membuat, menyetujui, dan mengekspor arsip vital tanpa mekanisme keseimbangan pengawasan.
...
Setiap log audit wajib mencatat: WHO (ID+Role), WHAT (aksi/view/edit/approve), WHEN (timestamp ISO 8601 presisi tinggi), WHERE (sumber/IP/lokasi sistem).

Untuk memenuhi standar PP 71/2019\footnote{PP 71/2019, Penyelenggaraan Sistem dan Transaksi Elektronik, LNRI 196 (2019).} \& Perka ANRI 6/2021\footnote{Perka ANRI 6/2021, Pengelolaan Arsip Elektronik, BNRI 541 (2021).}, terapkan 14 Field Metadata Audit:
...
Bagi Manajemen Atas, memahami apa yang harus dicatat lebih penting daripada memahami bagaimana sistem mencatatnya secara teknis. Manajemen Atas harus memastikan setiap kategori event berikut tercakup dalam mekanisme audit trail:

\begin{table}[htbp]
\centering
\small
\caption{Kategori Event Audit Trail untuk Arsip Digital}
\begin{tabularx}{\textwidth}{@{} l X X @{}}
\toprule
\textbf{Kategori} & \textbf{Jenis Event} & \textbf{Relevansi Bisnis} \\
\midrule
Akses & Login, logout, view, search & Memantau siapa yang mengakses arsip sensitif dan kapan \\
Perubahan & Create, update, rename, reclassify & Melacak setiap modifikasi konten dan metadata arsip \\
Persetujuan & Approve, reject, revoke & Menjamin akuntabilitas keputusan kritis atas arsip \\
Ekspor & Download, print, share, export & Mengendalikan dan memantau risiko kebocoran data \\
Sistem & Backup, restore, delete, archive & Mencatat operasi yang memengaruhi integritas dan ketersediaan \\
\bottomrule
\end{tabularx}
\end{table}
...
\caption{Konsep Hash Chain untuk Immutability Audit Log\\
\scriptsize Sumber: PT PLN (Persero).}
\label{fig:hash-chain}
\end{figure}

\small Gambar \ref{fig:hash-chain} mengilustrasikan bagaimana setiap entri log audit dihubungkan melalui hash chain untuk menjamin immutability. Setiap blok log (Log \#1 hingga Log \#5) berisi hash SHA-256 dari entri saat ini, referensi hash dari entri sebelumnya, serta metadata 3W (SIAPA: ID Pegawai + Role, APA: jenis aksi, KAPAN: timestamp). Perhatikan bahwa pada contoh ini, Log \#3 menunjukkan skenario pengubahan tidak sah: hash asli dicoret dan diganti, sehingga menyebabkan efek berantai -- Log \#4 dan Log \#5 berikutnya menampilkan status verifikasi gagal karena hash referensi mereka tidak lagi cocok. Sistem SIEM secara otomatis memicu peringatan PELANGGARAN INTEGRITAS TERDETEKSI sehingga Manajemen Atas dapat segera menindaklanjuti. Prinsip kuncinya: jika 1 byte data diubah, hash blok tersebut berubah, dan seluruh hash berikutnya gagal verifikasi sehingga manipulasi langsung terdeteksi.
...
\small Gambar \ref{fig:iso27001-controls} memetakan 93 kontrol dalam ISO/IEC 27001:2022 yang diorganisasi ke dalam 4 kelompok utama: Organizational Controls (37 kontrol), People Controls (8 kontrol), Physical Controls (14 kontrol), dan Technological Controls (34 kontrol). Indikator {\color{orange!80!black}$\star$} pada setiap sub-kategori menandakan relevansi \textbf{langsung} terhadap keamanan arsip digital — bukan sekadar kontrol keamanan umum. Sub-kategori yang disorot mencakup: Manajemen Aset (memastikan setiap arsip memiliki owner dan klasifikasi), Business Continuity (melindungi arsip dari bencana), Kontrol Akses (RBAC dan enkripsi), Kriptografi (enkripsi data arsip), Operasional Keamanan (monitoring dan backup), serta Manajemen Insiden (respons terhadap kebocoran data). \textbf{Relevansi strategisnya}: ketika Manajemen Atas menghadapi keterbatasan anggaran atau sumber daya, peta ini memberikan justifikasi obyektif untuk memprioritaskan investasi pada sub-kategori bertanda bintang. Sebaliknya, jika sub-kategori bertanda bintang belum terimplementasi dengan baik, organisasi menghadapi risiko kepatuhan yang signifikan terhadap PP 71/2019 dan Perka ANRI 6/2021, meskipun kontrol pendukung lainnya sudah lengkap.

\section{Tiga Prinsip Fundamental Kebijakan}

Sebelum membahas detail kontrol ISO 27001:2022 pada Gambar \ref{fig:iso27001-controls}, Manajemen Atas perlu memahami tiga prinsip fundamental yang menjadi landasan filosofis seluruh kebijakan keamanan arsip digital. Ketiga prinsip ini bukan konsep teknis, melainkan keputusan strategis yang harus diinternalisasi oleh Manajemen Atas sebagai basis pengambilan keputusan tata kelola keamanan informasi.
...
\caption{Arsitektur Zero Trust Referensi NIST -- Komponen Utama: Subject, PEP, PDP (PE + PA), PIPs, dan Resource\\
\small Sumber: NIST SP 1800-35 -- Implementing a Zero Trust Architecture, Figure 1 (Public Domain)
...
\caption{Model Zero Trust untuk Keamanan Arsip Digital PLN -- Arsitektur Lengkap dengan Control Plane dan Data Plane\\
\small Sumber: NIST SP 800-207 Zero Trust Architecture, diadaptasi untuk konteks PLN.}
\label{fig:zero-trust-pln}
\vfill
\end{figure}

\small Gambar \ref{fig:zero-trust-pln} mengadaptasi arsitektur Zero Trust NIST (Gambar 3.4) ke dalam konteks spesifik keamanan arsip digital PLN dengan arsitektur lengkap yang terdiri dari Control Plane (keputusan kebijakan) dan Data Plane (transmisi data). Pada Control Plane, Policy Engine (PE) dan Policy Administrator (PA) bekerja sama untuk mengevaluasi dan mengelola kebijakan akses. PE menerima data verifikasi dari PEP dan melakukan query ke empat Policy Information Points (PIPs): (1) PIP: ICAM untuk data identitas dan akses (SSO, MFA, RBAC), (2) PIP: Endpoint untuk keamanan perangkat (Device Trust, MDM), (3) PIP: Analytics untuk deteksi ancaman real-time, dan (4) PIP: Data Security untuk klasifikasi dan integritas data arsip.

Pada Data Plane, Subject (pegawai PLN dengan berbagai role: Staf Kearsipan, GM/SVP, Auditor SPI, Staf IT) yang telah terautentikasi melalui SSO + MFA + Device Trust mengajukan permintaan akses ke Policy Enforcement Point (PEP). PEP melakukan 4 tahap verifikasi awal: (1) identitas melalui RBAC, (2) integritas perangkat, (3) konteks lokasi dan waktu, serta (4) klasifikasi arsip yang diminta. Data verifikasi ini dikirim ke Control Plane untuk evaluasi mendalam.
...
Seluruh aktivitas dicatat dalam Audit Trail immutable dengan hash chain SHA-256, memungkinkan verifikasi berkelanjutan (Continuous Verification) sesuai prinsip Zero Trust NIST. Trust factors yang dievaluasi mencakup: role pengguna, integritas perangkat, lokasi geografis, waktu akses, sensitivitas data, dan konteks ancaman. Pendekatan ini sangat relevan untuk PLN dengan infrastruktur tersebar di seluruh Indonesia, memastikan bahwa tidak ada akses default dan setiap permintaan diverifikasi secara eksplisit berdasarkan konteks real-time.
...
\textbf{Akuntabilitas} & RACI digital jelas; setiap aksi terikat identitas dan TTE \\
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Transparansi (Transparency)} -- Kebijakan keamanan arsip harus dapat diakses dan dipahami oleh seluruh pemangku kepentingan yang berkepentingan, termasuk auditor eksternal. Hal ini tercermin dalam keterbukaan audit trail dan prosedur akses.
    \item \textbf{Akuntabilitas (Accountability)} -- Setiap aksi terhadap arsip harus dapat dipertanggungjawabkan kepada individu dan peran yang spesifik. Hal ini diterjemahkan melalui matriks RACI digital dan penggunaan Tanda Tangan Elektronik Tersertifikasi (TTE).
    \item \textbf{Responsibilitas (Responsibility)} -- Organisasi memiliki kewajiban untuk mematuhi regulasi kearsipan nasional (PP 71/2019, Perka ANRI 6/2021) dan membuktikan kepatuhan tersebut secara berkala melalui audit internal dan eksternal.
    \item \textbf{Independensi (Independence)} -- Unit Kearsipan dan fungsi pengawasan harus memiliki kewenangan untuk menolak arsip yang tidak memenuhi standar metadata, tanpa tekanan dari pihak manapun. Hal ini menjamin integritas sistem kearsipan secara keseluruhan.
    \item \textbf{Kewajaran (Fairness)} -- Kebijakan akses harus berbasis prinsip need-to-know dan tidak diskriminatif, dengan mekanisme banding bagi pengguna yang merasa hak aksesnya tidak sesuai.
\end{itemize}
...
Kebijakan keamanan arsip adalah payung hukum yang memberikan legitimasi dan keberlanjutan bagi seluruh mekanisme keamanan (RBAC, audit trail, backup). Tiga pesan kunci: (1) kebijakan harus terintegrasi dengan ISMS organisasi (ISO 27001:2022), bukan dokumen yang berdiri sendiri; (2) tiga prinsip fundamental -- Least Privilege, Defense in Depth, dan Zero Trust -- harus tercermin dalam setiap klausul kebijakan; dan (3) kebijakan yang tidak ditinjau secara berkala (minimal tahunan) akan menjadi usang dan kehilangan kekuatan hukumnya. Manajemen Atas adalah penandatangan dan penanggung jawab utama kebijakan ini.
\end{plnbox}
...
\begin{outputbox}
\textbf{Portofolio 3 (Pilar 3: Kebijakan Keamanan):} Rancangan Kebijakan Keamanan Arsip Terintegrasi ISMS. Dinilai berdasarkan kemampuan peserta merumuskan kebijakan keamanan yang terintegrasi dengan ISMS organisasi, keselarasan ISO 27001, kejelasan prinsip Zero Trust/Least Privilege, dan integrasi GCG BUMN.
\end{outputbox}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{321_rule.png}
...
\begin{itemize}[leftmargin=*]
    \item 3 salinan data (produksi + 2 backup)
    \item 2 media penyimpanan berbeda (misal: NAS lokal + Cloud/Offsite terenkripsi)
    \item 1 salinan disimpan di lokasi geografis terpisah dari data center utama
\end{itemize}
...
\caption{Timeline RTO dan RPO -- Hubungan antara Titik Pemulihan Data dan Waktu Pemulihan Sistem\\
\small Sumber: NIST SP 800-34 Rev.~1.}
\label{fig:rto-rpo}
\end{figure}

\small Gambar \ref{fig:rto-rpo} mengilustrasikan hubungan antara dua metrik kritis dalam perencanaan disaster recovery melalui sebuah garis waktu (timeline) horizontal. Untuk memahami diagram ini, mari ikuti alur dari kiri ke kanan.

Tahap 1 -- Sistem Beroperasi Normal. Pada awal timeline, garis kontinu menandakan bahwa sistem arsip digital beroperasi secara normal dan data tersedia (Available). Pada periode ini, terlihat dua titik Backup yang menunjukkan bahwa backup data dilakukan secara berkala -- misalnya setiap jam atau setiap hari, tergantung kebijakan unit. Semakin sering backup dilakukan, semakin kecil celah kehilangan data jika bencana terjadi.

Tahap 2 -- Terjadi Bencana. Di tengah diagram, sebuah ikon menandai momen Disaster -- yaitu saat sistem mengalami gangguan besar, baik akibat serangan siber, kegagalan perangkat keras, bencana alam, atau kesalahan manusia. Pada titik inilah garis timeline berubah menjadi putus-putus, menandakan transisi dari status Available (sistem berjalan) menjadi Not Available (sistem lumpuh). Selama periode ini, arsip digital tidak dapat diakses oleh pemangku kepentingan.

Tahap 3 -- Sistem Dipulihkan. Setelah masa pemulihan, titik Recovered menandai momen ketika sistem berhasil dipulihkan dan arsip kembali dapat diakses. Timeline kembali kontinu, mengindikasikan operasional normal telah ter restore.

Dari alur tersebut, dua metrik kunci dapat diidentifikasi melalui bracket (kurung pengukur) di bawah timeline:

\begin{itemize}[leftmargin=*]
    \item RPO (Recovery Point Objective) -- diukur oleh bracket yang membentang dari titik Backup terakhir menuju Recovery Point. RPO menjawab pertanyaan: ``Seberapa banyak data yang hilang?'' Jika backup terakhir dilakukan pukul 08.00 dan bencana terjadi pukul 11.30, maka RPO = 3,5 jam -- artinya seluruh data yang dibuat/ubah antara pukul 08.00--11.30 berpotensi hilang dan harus direkonstruksi ulang. Semakin kecil RPO, semakin sedikit data yang hilang -- tetapi membutuhkan frekuensi backup yang lebih tinggi dan biaya infrastruktur yang lebih besar.

    \item RTO (Recovery Time Objective) -- diukur oleh bracket yang membentang dari titik Disaster menuju Recovered. RTO menjawab pertanyaan: ``Seberapa lama sistem lumpuh total?'' Jika disaster terjadi pukul 11.30 dan sistem baru pulih pukul 15.30, maka RTO = 4 jam. Selama 4 jam tersebut, seluruh akses terhadap arsip digital PLN tidak tersedia -- karyawan tidak bisa mengunduh kontrak, auditor tidak bisa memeriksa laporan keuangan, dan manajemen tidak bisa mengakses dokumen RUPS. Semakin kecil RTO, semakin cepat pemulihan -- tetapi membutuhkan investasi pada teknologi \textit{failover} otomatis, \textit{mirror site}, dan tim respons yang terlatih.
\end{itemize}
...
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{2.2cm} P{2.5cm} P{2.5cm} X @{}}
\rowcolor{rowhead}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{gambar/BCP_Lifecycle_Custom.png}
\caption{Siklus Hidup Business Continuity Plan (BCP) -- Lima Fase Iteratif ISO 22301:2019\\
\small Sumber: ISO 22301:2019.}
\label{fig:bcp-lifecycle}
\end{figure}

\small Gambar \ref{fig:bcp-lifecycle} menunjukkan siklus hidup Business Continuity Planning (BCP) yang terdiri dari 5 fase iteratif, diurutkan searah jarum jam:

\begin{enumerate}[leftmargin=*]
    \item Maintenance -- pemeliharaan dan pembaruan rencana secara berkala. Fase ini memastikan bahwa BCP tetap relevan seiring berubahnya kondisi organisasi, infrastruktur, dan regulasi. Tanpa maintenance, rencana yang pernah sempurna akan menjadi usang dan tidak dapat diandalkan saat dibutuhkan.

    \item Analysis -- tahap yang menampung Business Impact Analysis (BIA) dan penilaian risiko. Fase ini ditandai dengan badge BIA pada diagram, menandakan bahwa BIA adalah aktivitas paling kritis dalam keseluruhan siklus BCP. Pada tahap ini, organisasi mengidentifikasi proses bisnis kritis, mengukur dampak ketidaktersediaan terhadap operasional dan kepatuhan regulasi, serta menetapkan RTO/RPO berdasarkan klasifikasi arsip (Vital, Rahasia, Internal/Publik).

    \item Solution Design -- perumusan strategi pemulihan berdasarkan temuan BIA. Output fase ini adalah strategi backup, infrastruktur \textit{failover}, dan mekanisme pemulihan yang proporsional dengan target RTO/RPO yang telah ditetapkan pada fase Analysis.

    \item Implementation -- penyusunan dan pengimplementasian rencana tanggap, termasuk pembuatan prosedur operasional standar (SOP), penunjukan tim respons, dan penerapan infrastruktur backup.

    \item Testing \& Acceptance -- pengujian, validasi, dan persetujuan rencana melalui simulasi dan uji pemulihan. Hasil pengujian menjadi umpan balik yang dialirkan kembali ke fase Maintenance untuk penyempurnaan berkelanjutan.
\end{enumerate}
...
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}p{1.5cm} X X X @{}}
\rowcolor{rowhead}
...
\renewcommand{\arraystretch}{1.6}
\begin{tabularx}{\textwidth}{@{} >{\centering\arraybackslash}p{1cm} >{\raggedright\arraybackslash}p{3.5cm} X X >{\centering\arraybackslash}p{1cm} @{}}
\rowcolor{rowhead}
...
\rowcolor{rowalt}
Pilar 1 & Merancang skema kontrol akses RBAC terintegrasi dengan sistem kepegawaian & Matriks RBAC + Prosedur Akses & Kejelasan peran, least privilege, integrasi HR/SSO, review berkala & 25\% \\
\rowcolor{white}
...
\rowcolor{rowalt}
Pilar 3 & Merumuskan kebijakan keamanan terintegrasi dengan ISMS organisasi & Rancangan Kebijakan Keamanan & Keselarasan ISO 27001, Zero Trust, TARIF GCG & 25\% \\
\rowcolor{white}
...
\midrule
\textbf{Kurang (< 50)} & Matriks peran tidak lengkap; tidak ada keterkaitan dengan sistem kepegawaian; prinsip \textit{least privilege} tidak diterapkan; tidak ada prosedur \textit{provisioning/de-provisioning}. \\
\textbf{Cukup (50--69)} & Matriks peran lengkap tetapi belum detail; keterkaitan dengan sistem kepegawaian disebutkan secara konseptual; ada prosedur tetapi belum operasional. \\
\textbf{Baik (70--84)} & Matriks peran lengkap dengan pemisahan hak akses yang jelas; prosedur \textit{provisioning/de-provisioning} terhubung dengan siklus hidup kepegawaian; ada mekanisme review berkala. \\
\textbf{Sangat Baik ($\geq$ 85)} & Seluruh indikator Baik terpenuhi, ditambah: validasi \textit{segregation of duties}, mekanisme akses sementara (delegasi, lintas-unit), dan penanganan mutasi darurat sudah tercakup. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Level} & \textbf{Indikator} \\
\midrule
\textbf{Kurang (< 50)} & Tidak ada daftar event yang diaudit; 14 field metadata tidak lengkap; tidak ada penjelasan mekanisme immutability; tidak ada mekanisme eskalasi. \\
\textbf{Cukup (50--69)} & Daftar event ada tetapi tidak terstruktur; field metadata terisi sebagian; immutability disebutkan tetapi mekanisme tidak jelas; eskalasi bersifat umum. \\
\textbf{Baik (70--84)} & Kategori event lengkap dan terstruktur (akses, perubahan, persetujuan, ekspor, sistem); 14 field terisi lengkap; mekanisme immutability (hash chain/WORM) dijelaskan; ada prosedur eskalasi dengan timeline. \\
\textbf{Sangat Baik ($\geq$ 85)} & Seluruh indikator Baik terpenuhi, ditambah: mekanisme alert otomatis untuk event kritis, retensi log sudah ditetapkan, dan kesesuaian dengan PP 71/2019 dan Perka ANRI 6/2021 dibuktikan. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Level} & \textbf{Indikator} \\
\midrule
\textbf{Kurang (< 50)} & Kebijakan tidak merujuk ISO 27001; tidak ada keterkaitan dengan ISMS organisasi; prinsip TARIF tidak terakomodasi; tidak ada analisis risiko. \\
\textbf{Cukup (50--69)} & Kebijakan merujuk ISO 27001 secara umum; ada keterkaitan dengan ISMS tetapi belum spesifik; sebagian prinsip TARIF terakomodasi; analisis risiko masih permukaan. \\
\textbf{Baik (70--84)} & Kebijakan selaras dengan minimum 3 kontrol ISO 27001 yang relevan; posisi kebijakan dalam hierarki ISMS jelas; seluruh prinsip TARIF terakomodasi; ada hasil analisis risiko sederhana. \\
\textbf{Sangat Baik ($\geq$ 85)} & Seluruh indikator Baik terpenuhi, ditambah: mekanisme review berkala terjadwal, proses konsultasi internal terdokumentasi, dan klausul kepatuhan terhadap regulasi nasional (PP 71/2019, UU PDP) sudah spesifik. \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Level} & \textbf{Indikator} \\
\midrule
\textbf{Kurang (< 50)} & Strategi backup tidak mengikuti 3-2-1; RTO/RPO tidak ditetapkan; tidak ada BCP; tidak ada prosedur fallback. \\
\textbf{Cukup (50--69)} & Strategi 3-2-1 disebutkan tetapi belum operasional; RTO/RPO ada tetapi belum berbasis BIA; ada draf BCP tetapi belum lengkap. \\
\textbf{Baik (70--84)} & Strategi 3-2-1 operasional; RTO/RPO berbasis klasifikasi arsip dan BIA sederhana; BCP mencakup prosedur \textit{fallback}, prioritas \textit{recovery}, dan jadwal simulasi. \\
\textbf{Sangat Baik ($\geq$ 85)} & Seluruh indikator Baik terpenuhi, ditambah: jadwal simulasi terdokumentasi dengan hasil evaluasi, mekanisme komunikasi krisis, dan keterkaitan dengan ISMS (backup sebagai kontrol keamanan) sudah dijelaskan. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{GATE VALUE:} Mitigasi risiko terkuantifikasi (\textit{downtime} $\downarrow$, temuan audit $\rightarrow 0$)?
    \item \textbf{GATE SCALE:} Infrastruktur \& prosedur siap diskalakan jangka panjang?
\end{enumerate}
Jika 1 gate bernilai No, lakukan perancangan ulang sebelum diajukan ke Komite Tata Kelola.
...
\begin{description}[leftmargin=2.5cm, style=nextline]
    ...
    \item[Backup] Pembuatan salinan data arsip yang disimpan secara terpisah dari data asli, untuk keperluan pemulihan jika terjadi kerusakan atau kehilangan data.
    \item[BCP] Business Continuity Plan -- rencana terstruktur yang memastikan organisasi dapat melanjutkan fungsi bisnis kritis meskipun menghadapi gangguan besar.
    \item[BIA] Business Impact Analysis -- analisis yang mengidentifikasi dampak ketidaktersediaan aset terhadap operasional, keuangan, dan kepatuhan, menjadi dasar penetapan RTO/RPO.
    \item[Break-Glass Procedure] Mekanisme akses darurat di luar hak akses normal, dengan pencatatan audit trail lengkap dan persetujuan atasan.
    ...
    \item[Hash Chain] Mekanisme yang menghubungkan setiap entri log secara kriptografis, sehingga perubahan satu entri menyebabkan seluruh rantai berikutnya gagal verifikasi.
    ...
    \item[Least Privilege] Prinsip keamanan yang menyatakan bahwa setiap pengguna hanya boleh memiliki hak akses minimum yang diperlukan untuk menjalankan tugasnya.
    ...
    \item[RBAC] Role-Based Access Control -- model kontrol akses berbasis peran di mana hak akses diberikan kepada jabatan/peran, bukan langsung kepada individu.
    ...
    \item[Segregation of Duties] Pemisahan fungsi yang memastikan tidak ada satu peran yang memiliki kombinasi hak akses berlebihan, untuk mencegah konflik kepentingan.
    ...
    \item[TARIF] Transparansi, Akuntabilitas, Responsibilitas, Independensi, Fairness (Kewajaran) -- lima prinsip tata kelola BUMN sebagaimana diatur dalam Permen BUMN PER-2/MBU/03/2023.
    ...
    \item[Zero Trust] Prinsip arsitektur keamanan yang tidak memberikan akses default -- setiap permintaan akses harus diverifikasi secara eksplisit berdasarkan konteks.
\end{description}
}
...
\begin{enumerate}[leftmargin=*, nosep]
    ...
    \item Veeam. (2024). 3-2-1 Backup Rule Explained. \url{https://www.veeam.com/blog/3-2-1-backup-rule.html}
