...
\begin{enumerate}[label=\Alph*., nosep]
    \item Menghapus seluruh dokumen fisik dan mewajibkan transisi total ke 100\% \textit{paperless}
...
\item KPI ``\textit{Tingkat Ketepatan Temu Kembali (Retrieval Accuracy Rate)}'' dalam pengukuran kinerja kearsipan mengukur:
\begin{enumerate}[label=\Alph*., nosep]
...
\item Dari 7 domain pada ``Daftar Control Diagnostik Kesiapan Digital'' yang diadaptasi dari PARBICA Guideline 13, sebutkan \textbf{2 domain} beserta masing-masing satu pertanyaan diagnostik yang paling relevan untuk unit kerja Anda.
...
\item Jelaskan perbedaan antara pola integrasi \textbf{\textit{Penangkapan Data Otomatis Berbasis Pemicu}} dan \textbf{\textit{SLA-Driven Escalation}} dalam konteks alur kerja kearsipan PLN.
...
\begin{enumerate}[nosep]
    ...
    \item Dokumen yang sudah ditandatangani di-scan, lalu diupload ke berkas bersama di server lokal.
...
\end{enumerate}
Rata-rata \textit{waktu siklus} dari inspeksi hingga arsip tersimpan: \textbf{5--7 hari kerja}. Dalam 12 bulan terakhir, 23\% arsip inspeksi tidak dapat ditemukan saat dibutuhkan untuk audit BPK.
\end{plnbox}

\vspace{0.3cm}

\begin{enumerate}[leftmargin=*, label=\textbf{D1.\alph*}]
    \item \textbf{(5 poin)} Identifikasi minimal \textbf{3 hambatan operasional (\textit{friction points})} dari prosedur di atas menggunakan prinsip \textit{Value-Added Focus}.
...
\noindent\rule{\textwidth}{0.3pt}
    ...
    \item \textbf{(5 poin)} Rancang alur kerja \textit{To-Be} yang mengeliminasi hambatan operasional tersebut. Gunakan konsep \textit{Penangkapan Data Otomatis Berbasis Pemicu} dan \textit{Digital Validation}. Jelaskan alurnya dalam format naratif atau diagram sederhana.
...
\noindent\rule{\textwidth}{0.3pt}
    ...
    \item \textbf{(5 poin)} Jika \textit{waktu siklus} berhasil direduksi dari 6 hari menjadi 1 hari, dan setiap hari keterlambatan menyebabkan potensi denda ketidakpatuhan Rp 500.000/dokumen, hitung estimasi penghematan tahunan untuk 384 laporan inspeksi/tahun (32 menara $\times$ 12 bulan).
...
3 & \textbf{B} & Melacak satu dokumen nyata dari awal hingga akhir proses, mencatat hambatan & 4.3.1 \\
...
\begin{itemize}[nosep]
    \item \textbf{\textit{Document Shadowing}} (2 poin): Menjadikan satu dokumen nyata sebagai objek pelacakan dari awal hingga akhir proses, mencatat hambatan dan durasi transisi.
...
\item \textbf{Perbedaan Penangkapan Berbasis Pemicu vs SLA-Driven (6 poin):}
\begin{itemize}[nosep]
    \item \textbf{\textit{Penangkapan Data Otomatis Berbasis Pemicu}} (3 poin): Status tertentu di sistem operasional \textbf{secara otomatis memicu} pembuatan draft arsip + metadata. Inisiasi oleh \textit{event sistem}.
...
\begin{itemize}[nosep]
    \item (3 poin) Komponen ini memastikan sistem tahu: ke berkas penyimpanan digital mana arsip dikunci, siapa yang dilarang membacanya (RBAC), dan kapan arsip boleh dimusnahkan sesuai JRA.
...
\begin{itemize}[nosep]
    \item (2 poin) \textbf{Penghematan Waktu Siklus:} Reduksi \textit{waktu siklus} proses kearsipan (hari $\rightarrow$ jam).
...
\item \textbf{3 Hambatan Operasional (5 poin):}\\
...
\begin{itemize}[nosep]
    ...
    \item Pengisian metadata manual yang inkonsisten (risiko kegagalan temu kembali / \textit{retrieval failure})
    \item Upload manual ke berkas bersama tanpa indeks terstruktur
\end{itemize}

\item \textbf{Alur To-Be (5 poin):}\\
Jawaban berkualitas harus memuat elemen berikut:
\begin{itemize}[nosep]
    \item (2 poin) \textbf{Pemicu}: Input inspeksi langsung di lapangan via perangkat mobile/tablet $\rightarrow$ status ``Selesai'' memicu auto-generate draft arsip + metadata otomatis (nama menara, tanggal, teknisi, kategori temuan)
...
\begin{itemize}[nosep]
    \item Reduksi \textit{waktu siklus}: 6 hari $\rightarrow$ 1 hari = penghematan 5 hari/dokumen
...
\begin{itemize}[nosep]
    ...
    \item (2--3 poin) \textbf{Strategi 2:} Pilot terbatas (\textit{small win demonstration}) -- jalankan di satu sub-unit selama 30 hari, tunjukkan hasil reduksi \textit{waktu siklus} dan temuan zero-error ke Manajer Senior sebagai bukti empiris.
\end{itemize}
